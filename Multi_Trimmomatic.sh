...
then
    ...
    echo "$0: Input files successfully located."
    echo "$0: Removing old symlinks, just to be safe."
    find -type l -delete
fi
...
then
    ...
    echo "Trimmed Single-End Read File exists, skipping this step."
fi
...
if [ ! -e PEreadsTrimmed_1P.fq.gz ] || [ ! -e PEreadsTrimmed_2P.fq.gz ] || [ ! -e PEreadsTrimmed_1U.fq.gz] || [ ! -e PEreadsTrimmed_2U.fq.gz]
    ...
    echo "Trimmed paired-end read files exist, skipping this step."
fi
